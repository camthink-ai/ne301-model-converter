...
from .ne301_config import NE301Version, generate_ne301_json_config, get_ne301_toolchain
...
class SecureTempManager:
    ...
    def create_secure_temp_dir(self, prefix: str) -> str:
        ...
        try:
            ...
            if stat.S_IMODE(current_mode) != 0o700:
                ...
                logger.debug(f"Correcting temp directory permissions: {temp_dir} (0o700)")

            # 注册到清理列表
            with self._lock:
                self.temp_dirs.append(temp_dir)

            logger.debug(f"Creating secure temp directory: {temp_dir}")
            return temp_dir
            ...
            raise RuntimeError(
                ...
                f"Suggestion: Please check system temp directory permissions, ensure /tmp is writable and has enough disk space."
            ) from e
    ...
    def cleanup_all(self) -> None:
        """清理所有临时目录（退出时自动调用）"""
        logger.info(f"Cleaning {len(self.temp_dirs)} temp directories...")
...
class DockerToolChainAdapter:
            ...
            with DockerToolChainAdapter._client_lock:
                if DockerToolChainAdapter._docker_client is None:
                    try:
                        ...
                        logger.info(f"Docker client initialized successfully, NE301 image: {self.ne301_image}")
                    except Exception as e:
    ...
    def _get_host_path(self, container_path: Path) -> Optional[str]:
        ...
        with DockerToolChainAdapter._path_cache_lock:
            if container_path_str in DockerToolChainAdapter._path_cache:
                ...
                if Path(cached_path).exists():
                    logger.debug(f"✓ Using cached host path: {cached_path}")
                    return cached_path
                else:
                    # 缓存失效，移除
        ...
        try:
            inspect_cmd = ["docker", "inspect", container_name, "--format", "{{json .Mounts}}"]
                ...
                for mount in all_mounts:
                    ...
                    if destination in ["/app/uploads", "/app/outputs"]:
                        ...
                        if inferred_path.exists():
                            ...
                            # 缓存结果
                            with DockerToolChainAdapter._path_cache_lock:
                                DockerToolChainAdapter._path_cache[container_path_str] = str(
                                    inferred_path
                                )
    ...
    def convert_model(
        ...
        logger.info(f"Starting conversion process for task {task_id}")
        task_manager.add_log(task_id, "Starting model conversion process...")
        ...
        try:
            # 执行完整转换流程
            task_manager.add_log(task_id, f"Model file: {Path(model_path).name}")
            task_manager.add_log(
                task_id, f"Input size: {config['input_size']}x{config['input_size']}"
            )

            model_size = Path(model_path).stat().st_size if Path(model_path).exists() else 0

            # ✅ 步骤 1+2: PyTorch → 量化 TFLite (0-60%) - 使用直接导出方法
            task_manager.add_log(task_id, "Step 1/3: Exporting quantized TFLite model")
            with self.performance_monitor.measure_step(task_id, "export_quantized_tflite"):
                if progress_callback:
                    progress_callback(10, "Exporting quantized TFLite model...")

                quantized_tflite = self._export_to_quantized_tflite(
                    task_id, model_path, config["input_size"], calib_dataset_path, yaml_path, config
                )
                logger.info(f"✅ Quantized TFLite export succeeded: {quantized_tflite}")
                task_manager.add_log(
                    task_id, f"✅ 量化 TFLite 导出完成: {Path(quantized_tflite).name}"
                )

            # 步骤 3: 准备 NE301 项目 (60-100%)
            # 💡 [核心修复] 使用全局锁保护 NE301 SDK 目录，防止并发冲突
            task_manager.add_log(task_id, "Step 2/3: Preparing and packaging NE301 project (exclusive mode)")
            with DockerToolChainAdapter._ne301_lock:
                # 步骤 3: 准备项目
                with self.performance_monitor.measure_step(task_id, "prepare_ne301"):
                    if progress_callback:
                        progress_callback(70, "Preparing NE301 project...")

                    ne301_project = self._prepare_ne301_project(
                        task_id, quantized_tflite, config, yaml_path
                    )
                    task_manager.add_log(task_id, "✅ NE301 project preparation completed")

                # 步骤 4: NE301 打包
                with self.performance_monitor.measure_step(task_id, "build_ne301"):
                    if progress_callback:
                        progress_callback(75, "Generating NE301 deployment package...")

                    try:
                        bin_path = self._build_ne301_model(
                            task_id, ne301_project, quantized_tflite  # ✅ 传递量化文件路径作为备选
                        )
                    finally:
            ...
            self.performance_monitor.end_task(
                task_id, success=True, model_size=model_size, output_size=output_size
            )

            task_manager.add_log(task_id, "✅ Conversion completed!")
            task_manager.add_log(task_id, f"Output file: {Path(bin_path).name}")
            logger.info(f"✅ Conversion succeeded: {bin_path}")
            return bin_path

        except Exception as e:
            logger.error(f"Conversion failed: {e}")
            task_manager.add_log(task_id, f"❌ Conversion failed: {str(e)}")
            # 记录失败
    ...
    def _extract_calibration_dataset(self, calib_dataset_path: str) -> str:
        ...
        logger.info(f"Detected calibration dataset as ZIP file, extracting...")
        ...
        try:
                ...
                for file_info in zip_ref.infolist():
                    ...
                    if file_info.external_attr:
                        ...
                        if stat.S_ISLNK(file_mode):
                            raise RuntimeError(
                                f"不允许符号链接: {file_path}。"
                                f"解决建议: 校准数据集 ZIP 文件不得包含符号链接，请重新打包。"
                            )
            ...
            logger.info(f"✅ Calibration dataset safely extracted to: {extract_dir}")
            ...
            if max_images > 0:
                logger.info(f"✅ Found best calibration image directory: {best_dir} (contains {max_images} images)")
                return best_dir

            logger.warning(f"No valid calibration images found after extraction")
            return extract_dir
            ...
            shutil.rmtree(extract_dir, ignore_errors=True)
            logger.error(f"Failed to extract calibration dataset: {e}")
            raise RuntimeError(
                f"解压校准数据集失败: {e}。"
                f"解决建议: 请确保 ZIP 文件格式正确，使用系统工具重新打包校准图片。"
            ) from e
    ...
    def _export_to_quantized_tflite(
        ...
        logger.info(f"Step 1: PyTorch → Quantized TFLite (Ultralytics native int8 approach)")
        logger.info(f"  Model: {model_path}")
        logger.info(f"  Input size: {input_size}x{input_size}")
        logger.info(f"  Quantization: Ultralytics int8, format=tflite")
        ...
        if calib_dataset_path:
            tm.add_log(task_id, "Extracting and optimizing calibration dataset...")
            actual_calib_path = self._extract_calibration_dataset(calib_dataset_path)

            # 再次检查目录中是否有图片
            images = [
                x
                for x in Path(actual_calib_path).iterdir()
                if x.suffix.lower() in (".jpg", ".jpeg", ".png", ".bmp")
            ]

            if not images:
                logger.warning("⚠️  No images found after extraction, preparing fallback data...")
        ...
        if not calib_dataset_path or not actual_calib_path:
            tm.add_log(task_id, "⚠️ No valid calibration set provided, generating temporary fallback data to ensure model compatibility...")
            ...
            logger.info(f"✅ Generated 10 {input_size}x{input_size} fallback images at {actual_calib_path}")
        ...
        if yaml_path and Path(yaml_path).exists():
            try:
                ...
                logger.warning(f"⚠️ Failed to read original YAML: {e}")
                # 极端兜底
                if nc > 0:
                    names_map = {i: f"class_{i}" for i in range(nc)}
        ...
        try:
            # 创建一个标准的临时数据集目录
            standard_dataset_dir = get_secure_temp_manager().create_secure_temp_dir(
                prefix="calib_std_"
            )
            ...
            source_imgs = [
                x
                for x in Path(str(actual_calib_path)).iterdir()
                if x.suffix.lower() in (".jpg", ".jpeg", ".png", ".bmp")
            ]
            ...
            logger.info(
                f"  ✅ 已构建标准校准数据集结构: {actual_dataset_root} ({len(source_imgs)} 张图片)"
            )
        except Exception as e:
            logger.warning(f"⚠️ Failed to build standard dataset structure: {e}, falling back to simple mode")
        ...
        logger.info(f"  ✅ Calibration dataset ready: {calib_img_count} images (with virtual labels)")
        tm.add_log(task_id, f"Calibration dataset ready: virtual labels added for Ultralytics compatibility")
        ...
        logger.info(f"  Ultralytics export args: {export_args}")
        logger.info(f"  Starting export (this may take a few minutes)...")
        tm.add_log(task_id, f"Executing Ultralytics TFLite int8 export (imgsz={input_size})...")
        ...
        try:
            ...
            logger.info(f"  ✅ Ultralytics export completed: {tflite_path}")
            ...
            if saved_model_dir.exists():
                try:
                    tm.add_log(task_id, "Performing forced uint8 full quantization (manually correcting input/output types)...")
                    logger.info(f"  Manually converting SavedModel -> uint8 TFLite: {saved_model_dir}")
                    ...
                    logger.info(f"  ✅ Manual forced uint8 conversion succeeded: {tflite_path}")
                    tm.add_log(task_id, "✅ Manual TFLite correction completed (uint8 input, int8 output)")

                except Exception as e:
                    logger.warning(f"⚠️  Manual conversion failed: {e}, will try scanning candidate files")
                    tm.add_log(task_id, f"⚠️ Manual correction failed: {e}, looking for alternative files")

            # 如果手动转换没成，扫描候选文件
            if not tflite_path.exists():
                candidates = (
                    list(saved_model_dir.glob("*full_integer_quant.tflite"))
                    if saved_model_dir.exists()
                    else []
                )
                if not candidates:
                ...
                if candidates:
                    ...
                    logger.info(f"  ✅ Found alternative TFLite file: {tflite_path}")
                else:
                    raise FileNotFoundError(
                        f"Ultralytics 导出后未找到 TFLite 文件: {tflite_path_raw}"
                    )
        ...
        logger.info(f"  TFLite file size: {tflite_path.stat().st_size:,} bytes")
        ...
        try:
            ...
            logger.info(f"    Input dtype: {input_dtype} (expected: uint8)")
            logger.info(f"    Output dtype: {output_dtype} (expected: int8)")
            logger.info(f"    Output shape: {output_shape}")

            if input_dtype != np.uint8:
                logger.error(f"❌ Input type incorrect: {input_dtype} (expected uint8)")
                raise ValueError(
                    f"量化后输入类型错误: {input_dtype}, 期望 uint8。"
                    f"解决建议: 请检查量化脚本配置，确保输入量化为 uint8 类型。"
                )

            if output_dtype != np.int8:
                logger.error(f"❌ Output type incorrect: {output_dtype} (expected int8)")
                raise ValueError(
                    f"量化后输出类型错误: {output_dtype}, 期望 int8。"
                    f"解决建议: 请检查量化脚本配置，确保输出量化为 int8 类型。"
                )
            ...
            if len(scales) > 0:
                logger.info(f"    ✅ Output scale: {scales[0]:.6f}  (actual quantization params)")
                logger.info(f"    ✅ Output zero_point: {zero_points[0]}")
            else:
                logger.warning("    ⚠️  Output quantization_parameters not found, JSON config will use default values")
            ...
            if expected:
                actual_boxes = (
                    int(output_shape[2]) if len(output_shape) > 2 else int(output_shape[1])
                )
                if actual_boxes == expected:
                    logger.info(f"    ✅ total_boxes correct: {actual_boxes}")
                else:
                    logger.error(f"    ❌ total_boxes incorrect: {actual_boxes} (expected {expected})")
                    raise ValueError(
                        f"输出形状错误: total_boxes={actual_boxes}, 期望 {expected}。"
                        f"解决建议: 请检查模型输出层配置，确保输出维度正确。"
                    )

            logger.info(f"  ✅ Quantized TFLite validation passed!")
            tm.add_log(
                task_id, f"✅ TFLite validation passed: input=uint8, output=int8, shape={list(output_shape)}"
            )

        except (ValueError, RuntimeError):
            raise
        except Exception as e:
            logger.warning(f"⚠️  Exception during validation: {e}")
            logger.warning("Continuing, but manual TFLite model check recommended")
    ...
    def _prepare_ne301_project(
        ...
        """
        logger.info("Step 3: Preparing NE301 project")
        ...
        logger.info(f"📏 Input size validation:")
        logger.info(f"  TFLite actual: {tflite_input_size}x{tflite_input_size}")
        logger.info(f"  Config setting: {config_input_size}x{config_input_size}")

        if tflite_input_size > 0 and tflite_input_size != config_input_size:
            logger.error(f"❌ Input size mismatch!")
            logger.error(
                f"  This will cause incorrect bin file size (expected ~{config_input_size*3//256} MB -> actual ~{tflite_input_size*3//256} MB)"
            )
            logger.error(f"  JSON config input size will be incorrect!")
        ...
        logger.info(f"✅ Input size validation passed: {config_input_size}x{config_input_size}")
        ...
        if yaml_path and Path(yaml_path).exists():
            try:
                ...
                if "names" in yaml_data:
                    ...
                    logger.info(f"✅ Read {len(class_names)} classes from YAML 'names' field")
                elif "classes" in yaml_data:
                    class_names = [
                        cls["name"] if isinstance(cls, dict) else str(cls)
                        for cls in yaml_data["classes"]
                    ]
                    logger.info(f"✅ Read {len(class_names)} classes from YAML 'classes' field")
                ...
                logger.warning(f"⚠️  Failed to read YAML file: {e}, will use empty class list")
        ...
        if class_names:
            ...
            logger.info(f"✅ num_classes aligned: {num_classes_from_config}")
        else:
            logger.warning(f"⚠️  No YAML file provided or no class info")
            logger.warning(f"  Using config value: {num_classes_from_config}")
        ...
        logger.info(f"✅ TFLite model copied to weights directory: {ne301_tflite_path}")
        ...
        logger.info(f"✅ JSON file written to weights directory: {json_file} ({file_size} bytes)")
        ...
        if model_makefile.exists():
            ...
            if new_content != makefile_content:
                ...
                logger.info(f"✅ Model/Makefile updated (MODEL_NAME and RELOC_CONFIG)")
            else:
                logger.warning(f"⚠️  Config lines not found in Model/Makefile")
        else:
            logger.warning(f"⚠️  Model/Makefile not found: {model_makefile}")
        ...
        if mpool_file.exists():
            try:
                ...
                logger.info(f"📋 mpool config diagnosis:")
                logger.info(
                    f"  - xSPI1 (hyperRAM): size={xspi1_config['size']['value']}{xspi1_config['size']['magnitude']}, constants_preferred={xspi1_config['constants_preferred']}"
                )
                logger.info(
                    f"  - xSPI2 (octoFlash): size={xspi2_config['size']['value']}{xspi2_config['size']['magnitude']}, constants_preferred={xspi2_config['constants_preferred']}"
                )
                ...
                if xspi2_size_mb == 0 and xspi2_const_pref == "true":
                    logger.warning("⚠️  Detected mpool config issue:")
                    logger.warning("  💡 xSPI2 size=0 but constants_preferred=true")
                    logger.warning("  💡 This causes compiler to select xSPI2 but cannot allocate memory")
                    logger.warning("  💡 Fix: changing xSPI2 constants_preferred to false")
                    ...
                    logger.info("✅ Fixed mpool config: xSPI2 constants_preferred -> false")
                    logger.info("✅ Compiler will use xSPI1 (8MB RAM) for model parameters")
                else:
                    logger.info("✅ mpool config correct, no changes needed")

            except Exception as e:
                logger.warning(f"⚠️  mpool config processing failed: {e}")
        else:
            logger.warning(f"⚠️  mpool file not found: {mpool_file}")

        logger.info(f"✅ Model file prepared: {model_name}.tflite")
        logger.info(f"✅ JSON config generated: {model_name}.json")
        logger.info(
            f"✅ Config params: input_size={config['input_size']}, num_classes={config['num_classes']}"
        )

        # 返回 Model 目录路径
        model_project_dir = ne301_project / "Model"
        logger.info(f"✅ NE301 project preparation completed: {model_project_dir}")
    ...
    def _extract_input_size_from_tflite(self, tflite_path: str) -> int:
        ...
        try:
            ...
            logger.info(f"🔍 Extracting input size from TFLite: {tflite_path}")

            interpreter = tf.lite.Interpreter(model_path=tflite_path)
            input_details = interpreter.get_input_details()

            if not input_details:
                logger.warning("⚠️  TFLite model has no input tensor")
                return -1
            ...
            if len(input_shape) != 4:
                logger.warning(f"⚠️  Input shape is not 4D: {input_shape}")
                return -1
            ...
            if height != width:
                logger.warning(f"⚠️  Input is not square: {height}x{width}")

            logger.info(f"✅ TFLite input size: {height}x{width}")
            return height

        except ImportError:
            logger.warning("⚠️  TensorFlow not installed, cannot verify TFLite input size")
            return -1
        except Exception as e:
            logger.warning(f"⚠️  Cannot extract input size from TFLite: {e}")
            return -1

    def _build_ne301_model(
        self, task_id: str, ne301_project_path: Path, quantized_tflite: str
    ) -> str:
        ...
        """
        logger.info(f"Step 4: Calling NE301 container for packaging")
        ...
        if is_arm64:
            logger.info(f"ℹ️  Detected ARM64 architecture (Apple Silicon)")
            logger.info(f"ℹ️  Will use QEMU emulation for NE301 packaging")
            logger.info(f"ℹ️  First run may be slow, subsequent runs will cache translation results")
        else:
            logger.info(f"ℹ️  Detected x86_64 architecture, will use native performance")

        # 尝试 NE301 打包（无论架构如何）
        try:
            return self._attempt_ne301_build(
                task_id, ne301_project_path, quantized_tflite, model_name
            )
        except RuntimeError as e:
            logger.error(f"❌ NE301 packaging failed: {e}")
            logger.info("📦 Providing quantized TFLite as fallback output...")
            return self._provide_quantized_tflite_output(task_id, quantized_tflite)

    def _attempt_ne301_build(
        self, task_id: str, ne301_project_path: Path, quantized_tflite: str, model_name: str
    ) -> str:
        ...
        """
        logger.info("📦 NE301 packaging process")
        logger.info(f"  Project path: {ne301_project_path}")
        ...
        logger.info(f"  Selected packaging method: {packaging_method}")
        ...
        try:
            ...
            logger.error(f"❌ {packaging_method} packaging failed: {e}")
            logger.info("📦 Falling back to TFLite output...")
            return self._provide_fallback_output(task_id, quantized_tflite)

    def _build_ota_package(
        self, task_id: str, ne301_project_path: Path, toolchain, model_name: str
    ) -> str:
        """OTA 固件打包（推荐方式）

        生成带 OTA 头部的完整固件，兼容设备升级
        """
        logger.info("🎯 Using OTA firmware packaging (recommended)")
        ...
        logger.info(f"✅ OTA firmware generated: {Path(pkg_bin_path).name}")
    ...
    def _build_model_package(
        ...
        """
        logger.info("🎯 Using pure model packaging (fallback)")

        # 执行 make model
        # 💡 传入版本信息
        model_bin_path = self._make_model(
            task_id, ne301_project_path, model_name, toolchain.version
        )
    ...
    def _make_model(
        ...
        logger.info(f"  Project path: {ne301_project_path}")
        logger.info(f"  Using NE301 image: {self.ne301_image}")
        logger.info(f"  Model name: {model_name}")
        ...
        if not host_project_path:
            logger.warning(
                f"  ⚠️ 无法获取 {self.ne301_project_path} 的宿主机路径，回退到普通卷处理"
            )
            ...
            logger.info(f"  ✓ Mounted host path: {host_project_path} -> /workspace")
        ...
        if build_dir.exists():
            logger.info(f"  🧹 Cleaning old packages in build directory: {build_dir}")
            for old_pkg in build_dir.glob("*_pkg.bin"):
                try:
                    old_pkg.unlink()
                except Exception as e:
                    logger.warning(f"  ⚠️ Cannot delete old package {old_pkg}: {e}")
        ...
        logger.info(f"  Starting NE301 container: {container_name}")
        logger.info(f"  Executing command: {' '.join(make_cmd)}")
        logger.info(f"  Model name (MODEL_NAME): {model_name}")
        logger.info(f"  Volume mount: ne301_workspace -> /workspace")
        ...
        try:
            ...
            logger.info(f"  ✓ NE301 container started: {container.id[:12]}")
            tm.add_log(task_id, f"Container started, starting compilation and packaging (ID: {container.id[:12]})...")
            ...
            def stream_logs():
                try:
                    ...
                    for line in container.logs(stream=True, follow=True):
                        ...
                        if log_line:
                            # 🔧 修复：移除 ANSI 颜色代码，避免前端解析问题
                            # ANSI escape sequence pattern: \x1b[<code>m
                            ansi_escape = re.compile(r'\x1b\[[0-9;]*m')
                            clean_log = ansi_escape.sub('', log_line)

                            # 🔧 增强：同时打印到后端日志（方便调试）
                            logger.debug(f"[NE301] {clean_log}")

                            # 推送到前端
                            tm.add_log(task_id, clean_log)
                except Exception as e:
                    logger.error(f"Log stream interrupted: {e}")
                    tm.add_log(task_id, f"⚠️ 日志流中断: {str(e)}")
            ...
            logger.info(f"  Container exited, exit code: {result.get('StatusCode')}")
            ...
            target_pkg_files = sorted(
                build_dir.glob("*_pkg.bin"), key=lambda p: p.stat().st_mtime, reverse=True
            )

            target_output = None
            if target_pkg_files:
                target_output = target_pkg_files[0]
                logger.info(f"  ✓ Captured newly generated package: {target_output.name}")

            if target_output and target_output.exists():
                # 找到目标文件
                file_size = target_output.stat().st_size
                logger.info(
                    f"  ✓ make process completed, output file: {target_output.name} ({file_size:,} bytes)"
                )
                tm.add_log(
                    task_id, f"✅ NE301 packaging completed: {target_output.name} ({file_size // 1024}KB)"
                )
                ...
                logger.info(f"  ✓ Output file copied to: {output_bin}")
                ...
                if result["StatusCode"] != 0:
                    logger.error(f"  ✗ make failed, exit code: {result['StatusCode']}")
                    raise RuntimeError(
                        f"make pkg-model failed (exit code {result['StatusCode']}), no output file generated."
                    )
                else:
                    logger.error(f"  ✗ make succeeded but no *_pkg.bin file found in build/ directory")
                    logger.error(
                        f"  build/ directory contents: {list(build_dir.iterdir()) if build_dir.exists() else 'dir not found'}"
                    )
                    raise RuntimeError(
                        "make pkg-model 成功但输出文件未生成。"
                        "解决建议: 请检查 NE301 SDK 配置，确保 build/ 目录存在且可写。"
                        "可能需要清理 build/ 目录后重新尝试。"
                    )
            ...
            if container:
                try:
                    ...
                    logger.info(f"  ✓ Container cleaned: {container_name}")
                except Exception as e:
                    logger.warning(f"  Container cleanup failed (may already be deleted): {e}")
    ...
    def _run_make_directly(self, ne301_project_path: Path) -> None:
        ...
        try:
            ...
            if result.returncode != 0:
                ...
                raise RuntimeError(
                    f"make model failed: {result.stderr}. "
                    f"Suggestion: Check NE301 project configuration, ensure Makefile and dependencies are correct. "
                    f"Try manually entering the container and running 'make model' for debugging."
                )

            logger.info("✅ make model completed")
            if result.stdout:
                logger.debug(f"make output: {result.stdout[-500:]}")  # 只显示最后500字符

        except subprocess.TimeoutExpired:
            raise RuntimeError("make model timeout (>120s)")
    ...
    def _run_make_in_container(self, ne301_project_path: Path) -> None:
        ...
        try:
            ...
            # 如果容器不存在，尝试创建临时容器
            logger.info("Creating temporary NE301 container...")
            ne301_container = self._run_temp_ne301_container(ne301_project_path)

        # 执行 make
        try:
            exit_code, output = ne301_container.exec_run(
                f"bash -c 'cd {ne301_project_path} && make model'", workdir=str(ne301_project_path)
            )

            if exit_code != 0:
                logger.error(f"make failed: {output.decode('utf-8')[-500:]}")
                raise RuntimeError(
                    f"make model failed: exit code {exit_code}. "
                    f"Suggestion: Check NE301 container status, ensure dependencies are complete inside container. "
                    f"View detailed logs for specific error reasons."
                )

            logger.info("✅ make model completed")

        except Exception as e:
            logger.error(f"Failed to execute make: {e}")
            raise
    ...
    def _run_temp_ne301_container(self, ne301_project_path: Path):
        ...
        # 创建卷挂载
        volumes = {str(ne301_project_path): {"bind": str(ne301_project_path), "mode": "rw"}}
    ...
    def _add_ota_header(
        self, task_id: str, ne301_project_path: Path, toolchain, model_bin_path: Path
    ) -> str:
        """添加 OTA 固件头部"""
        logger.info("Step 2: Adding OTA firmware header...")
        ...
        logger.info(f"  Output: {ota_pkg_path.name}")
        ...
        if process.returncode != 0:
            raise RuntimeError(
                f"OTA packaging failed: {' '.join(output[-10:])}. "
                f"Suggestion: Check NE301 SDK version, ensure OTA packaging tool is available and configured correctly."
            )
        ...
        logger.info(f"✅ OTA firmware generated successfully: {final_path}")
        return str(final_path)

    def _provide_fallback_output(self, task_id: str, quantized_tflite: str) -> str:
        """提供降级输出（TFLite）"""
        logger.info("⚠️  Falling back to TFLite output")
        ...
        logger.info(f"✅ TFLite file provided: {fallback_path}")
        logger.warning("⚠️  Note: TFLite format needs manual packaging to NE301 format")
    ...
    def _cleanup_ne301_sdk_artifacts(self, task_id: str, ne301_project_path: Path) -> None:
        ...
        # 尝试清理 TFLite 和 JSON
        files_to_remove = [weights_dir / f"{model_name}.tflite", weights_dir / f"{model_name}.json"]

        for f in files_to_remove:
            try:
                if f.exists():
                    f.unlink()
                    logger.debug(f"  🧹 Cleaned SDK temp file: {f.name}")
            except Exception as e:
                logger.warning(f"  ⚠️  Cannot clean SDK temp file {f}: {e}")

    def _provide_quantized_tflite_output(self, task_id: str, quantized_tflite: str) -> str:
        ...
        """
        logger.info("📦 Preparing quantized TFLite output...")

        # 检查量化文件是否存在
        tflite_path = Path(quantized_tflite)
        if not tflite_path.exists():
            raise FileNotFoundError(f"Quantized TFLite file not found: {quantized_tflite}")
        ...
        logger.info(f"✅ Quantized TFLite generated: {final_tflite_path}")
        logger.warning("⚠️  Note: This file is quantized TFLite format, not NE301 .bin format")
        logger.info("💡 Tip: To complete NE301 packaging in x86_64 environment, follow these steps:")
        logger.info("   1. Transfer quantized TFLite file to x86_64 server")
        logger.info("   2. Run NE301 packaging command on that server")
        logger.info("   3. Or use cloud services for packaging (AWS/GCP/Azure x86_64 instances)")
    ...
    def _export_to_saved_model(self, model_path: str, input_size: int, task_id: str) -> str:
        ...
        logger.info(f"[{task_id}] Step 1: Exporting SavedModel format")
        logger.info(f"[{task_id}]   Model: {model_path}")
        logger.info(f"[{task_id}]   Input size: {input_size}x{input_size}")
        ...
        if model_path_obj.suffix.lower() not in allowed_extensions:
            raise ValueError(
                f"Unsupported model file format: {model_path_obj.suffix}. "
        ...
        if file_size > MAX_MODEL_SIZE:
            raise ValueError(
                ...
                f"Maximum supported: {MAX_MODEL_SIZE / 1024 / 1024}MB"
            )

        logger.info(f"[{task_id}] ✅ Model file validation passed: {file_size / 1024 / 1024:.2f}MB")

        try:
            # 加载 YOLO 模型
            # 注意: Ultralytics YOLO 不支持 weights_only 参数，但已通过上述验证降低风险
            logger.info(f"[{task_id}] Loading model...")
            model = YOLO(model_path)
            ...
            logger.info(f"[{task_id}] Exporting SavedModel...")
            model.export(
                format="saved_model",
                imgsz=input_size,
            ...
            logger.info(f"[{task_id}] ✅ SavedModel export succeeded: {export_path}")
            return str(export_path)

        except Exception as e:
            logger.error(f"[{task_id}] ❌ SavedModel export failed: {e}")
            raise RuntimeError(
                f"SavedModel export failed: {e}. "
                f"Suggestion: Check if model file is corrupted, ensure it's a valid PyTorch/ONNX model. "
                f"If it's a YOLO model, ensure using Ultralytics export."
            ) from e

    def _prepare_quant_config(
        self, saved_model_dir: str, input_size: int, calib_dataset_path: Optional[str], task_id: str
    ) -> Path:
        """准备 ST 量化配置文件 (YAML)
        ...
        logger.info(f"[{task_id}] Step 2: Preparing quantization config file")
        ...
        if not actual_calib_path:
            logger.warning(f"[{task_id}] ⚠️  No calibration dataset provided!")
            logger.warning(f"[{task_id}]    Real quantization requires calibration dataset for activation distribution statistics")
            logger.warning(f"[{task_id}]    Will use fake quantization mode (accuracy may degrade)")
        ...
        logger.info(f"[{task_id}] ✅ Config file generated: {config_path}")
        if actual_calib_path:
            logger.info(f"[{task_id}]   Using calibration dataset: {actual_calib_path}")
            logger.info(f"[{task_id}]   Quantization mode: Real quantization (fake=False)")
        else:
            logger.info(f"[{task_id}]   Quantization mode: Fake quantization (fake=True, accuracy may degrade)")

        return config_path

    def _run_st_quantization(self, config_path: Path, task_id: str) -> str:
        """运行 ST 官方量化脚本（安全版本）
        ...
        """
        logger.info(f"[{task_id}] Step 3: Running ST official quantization script")
        ...
        if not quant_script.exists():
            raise RuntimeError(
                f"量化脚本不存在: {quant_script}。"
                f"解决建议: 请确保 backend/tools/quantization/tflite_quant.py 文件存在。"
                f"如果缺失，请从项目仓库重新获取。"
            )
        ...
        try:
            ...
            if result.returncode != 0:
                ...
                logger.error(f"[{task_id}] ❌ Quantization script execution failed:")
                logger.error(f"[{task_id}]   Exit code: {result.returncode}")
                logger.error(f"[{task_id}]   Error output:\n{error_output}")
                raise RuntimeError(
                    f"Quantization failed: {error_output}. "
                    f"Suggestion: Check calibration dataset format, ensure valid JPG/PNG images are included. "
                    f"Can also try without calibration dataset using fake quantization mode."
                )

            # 查找生成的量化文件
            output_dir = Path(config_dir) / "quantized_models"
            if not output_dir.exists():
                raise FileNotFoundError(f"Quantization output directory not found: {output_dir}")

            # 查找 .tflite 文件
            tflite_files = list(output_dir.glob("*.tflite"))
            if not tflite_files:
                raise FileNotFoundError(f"Quantized file not generated (directory: {output_dir})")

            # 返回最新的文件
            quantized_file = max(tflite_files, key=lambda p: p.stat().st_mtime)
            logger.info(f"[{task_id}] ✅ Quantization completed: {quantized_file}")

            return str(quantized_file)

        except subprocess.TimeoutExpired:
            logger.error(f"[{task_id}] ❌ Quantization timeout (>10 min)")
            raise RuntimeError(
                "Quantization timeout (>10 min). "
                "Suggestion: Reduce calibration dataset size or use a higher-spec server. "
                "Recommended to use 32-100 representative images for calibration."
            )
        except Exception as e:
            logger.error(f"[{task_id}] ❌ Quantization failed: {e}")
            raise RuntimeError(
                f"Quantization failed: {e}. "
                f"Suggestion: Check model format and config params, ensure compatibility with quantization script."
            ) from e

    def _validate_quantized_model(
        self, quantized_tflite_path: str, input_size: int, task_id: str
    ) -> bool:
        """验证量化后的模型
        ...
        logger.info(f"[{task_id}] Step 4: Validating quantized model")

        # 验证文件存在
        if not Path(quantized_tflite_path).exists():
            raise FileNotFoundError(f"Quantized model file not found: {quantized_tflite_path}")
        ...
        try:
            ...
            logger.info(f"[{task_id}]   Input shape: {input_details['shape']}")
            logger.info(f"[{task_id}]   Output shape: {output_details['shape']}")
            ...
            if input_size in expected_boxes:
                ...
                if actual_boxes != expected:
                    error_msg = f"Output shape incorrect: expected (1, 34, {expected}), " f"actual {output_shape}"
                    logger.error(f"[{task_id}] ❌ {error_msg}")
                    raise ValueError(error_msg)

                logger.info(f"[{task_id}] ✅ Output shape correct (total_boxes={actual_boxes})")
            else:
                logger.warning(f"[{task_id}] ⚠️  Unknown input size {input_size}, skipping shape validation")
            ...
            if "quantization_parameters" in output_details:
                ...
                if scales and len(scales) > 0:
                    ...
                    logger.info(f"[{task_id}]   Quantization scale: {scale}")

                    # 检查 scale 是否合理（不应该是 1.0，除非是 float32）
                    if scale == 1.0 and output_details.get("dtype") != np.float32:
                        logger.warning(f"[{task_id}] ⚠️  Quantization scale is 1.0, may not be correctly quantized")
                else:
                    logger.warning(f"[{task_id}] ⚠️  Quantization params not found")
            else:
                logger.warning(f"[{task_id}] ⚠️  Model output has no quantization params")

            logger.info(f"[{task_id}] ✅ Model validation passed")
            return True
            ...
            if "TFLite" in str(e) or "interpreter" in str(e):
                raise RuntimeError(
                    f"Invalid TFLite model: {e}. "
                    f"Suggestion: Check TFLite export steps, ensure model is correctly exported. "
                    f"Can try re-exporting model and check TensorFlow version compatibility."
                ) from e
            raise
    ...
    def _convert_with_saved_model_and_st_quant(
        ...
        # 步骤 1: 导出 SavedModel
        task_manager = get_task_manager()
        task_manager.add_log(task_id, "Step 1/4: Exporting SavedModel format")

        if progress_callback:
            progress_callback(10, "Exporting SavedModel...")

        saved_model_dir = self._export_to_saved_model(
            model_path=model_path, input_size=input_size, task_id=task_id
        )

        # 步骤 2: 准备量化配置
        task_manager.add_log(task_id, "Step 2/4: Preparing quantization config")

        if progress_callback:
            progress_callback(20, "Preparing quantization config...")
        ...
        # 步骤 3: 运行 ST 量化
        task_manager.add_log(task_id, "Step 3/4: Running ST official quantization")

        if progress_callback:
            progress_callback(30, "Running ST quantization...")

        quantized_tflite = self._run_st_quantization(config_path=config_path, task_id=task_id)

        # 步骤 4: 验证量化模型
        task_manager.add_log(task_id, "Step 4/4: Validating quantized model")

        if progress_callback:
            progress_callback(50, "Validating quantized model...")

        self._validate_quantized_model(
            quantized_tflite_path=quantized_tflite, input_size=input_size, task_id=task_id
        )
