    ...
    def check(self) -> EnvironmentStatus:
        ...
        if not self.toolchain.check_image():
            return EnvironmentStatus(
                ...
                mode="docker",
                message="Docker ready, toolchain image will be pulled automatically on first conversion",
                image_size="~3GB",
                estimated_time="3-5 minutes (depends on network speed)",
            )

        # 3. 环境完全就绪
        return EnvironmentStatus(status="ready", mode="docker", message="Environment ready, conversion can start")
