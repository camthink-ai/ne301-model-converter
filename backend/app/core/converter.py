...
class ModelConverter:
    ...
    def convert(
        ...
        if not available:
            raise RuntimeError(f"Docker unavailable: {error}")

        # 检查镜像
        if not docker.check_image():
            logger.info("Docker image not found, pulling...")
            if progress_callback:
                progress_callback(5, "Pulling Docker image...")

            success = docker.pull_image(
                progress_callback=lambda p: progress_callback(5 + p // 20, "Pulling Docker image...") if progress_callback else None
            )
            ...
            if progress_callback:
                progress_callback(10, "Docker image ready")

        # 执行完整转换流程
        if progress_callback:
            progress_callback(15, "Starting model conversion...")
