...
class NE301Toolchain:
    ...
    def _detect_toolchain(self) -> None:
        """检测 NE301 工具链版本和可用工具"""
        logger.info("🔍 Detecting NE301 toolchain...")
    ...
    def _parse_version_mk(self, version_mk_path: Path) -> None:
        ...
        try:
            ...
            if major is not None and minor is not None and patch is not None:
                ...
                logger.info(f"✓ Detected NE301 version: {self.version}")

        except Exception as e:
            logger.warning(f"⚠️  Failed to parse version file: {e}")
    ...
    def _log_detection_results(self) -> None:
        """记录检测结果摘要"""
        logger.info("=" * 50)
        logger.info("NE301 toolchain detection results:")
        logger.info(f"  Project path: {self.project_root}")
        logger.info(f"  Version: {self.version or 'unknown'}")
    ...
    def get_model_version(self) -> NE301Version:
        ...
        if not version_mk.exists():
            logger.warning(f"⚠️  version.mk not found, using default version 3.0.0.1")
        ...
        try:
            ...
            if model_version_match:
                ...
                if len(parts) == 4:
                    ...
                    logger.info(f"✅ Read MODEL_VERSION_OVERRIDE from version.mk: {version}")
                    return version

            # ✅ 回退：读取主版本号
            logger.info("MODEL_VERSION_OVERRIDE not defined, using main version")
            ...
            version = NE301Version(major, minor, patch, build)
            logger.info(f"✅ Read main version from version.mk: {version}")
            return version

        except Exception as e:
            logger.warning(f"⚠️  Failed to read version.mk: {e}, using default version")
            return NE301Version(3, 0, 0, 1)
...
def extract_tflite_quantization_params(
    ...
    try:
        ...
        logger.warning(f"Failed to extract parameters from TFLite model: {e}", exc_info=True)
        return None, None, None, None
