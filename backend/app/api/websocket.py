...
class ConnectionManager:
    ...
    async def connect(self, websocket: WebSocket, task_id: str):
        ...
        logger.info(f"WebSocket connection established: task_id={task_id}")
    ...
    async def broadcast_to_task(self, task_id: str, message: dict):
        ...
        for connection in connections:
            try:
                ...
                logger.error(f"Broadcast message failed (task_id={task_id}): {e}")
                # 移除失败的连接
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        ...
        logger.info("WebSocket connection accepted, waiting for client message...")

        # 第二步：等待客户端发送订阅消息
        logger.info("Waiting for client subscription message...")
        data = await websocket.receive_json()
        logger.info(f"Received client message: {data}")
        action = data.get("action")
        task_id = data.get("task_id")
        logger.info(f"Parsed message: action={action}, task_id={task_id}")
        ...
        if action == "subscribe" and task_id:
            ...
            if task:
                ...
                if task.logs:
                    logger.info(f"Replaying {len(task.logs)} historical logs to task_id={task_id}")
            ...
            if task_id:
                await manager.send_personal_message(
                    {"type": "cancelled", "task_id": task_id, "data": {"message": "Subscription cancelled"}},
                    websocket,
                )
