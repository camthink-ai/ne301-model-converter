...
async def get_task_status(task_id: str):
    ...
    logger.info(f"Query task status: {task_id} - {task.status}")
...
async def list_tasks():
    ...
    logger.info(f"List all tasks: {len(tasks)} total")
...
async def download_task_output(task_id: str):
    ...
    if not output_path.exists():
        ...
        raise HTTPException(status_code=404, detail=f"Output file not found: {output_path.name}")
    ...
    logger.info(f"Download file: {filename} ({output_path.stat().st_size} bytes)")
...
async def get_performance_stats():
    ...
    logger.info(f"Query performance stats: {stats['total_tasks']} tasks")
...
async def get_task_stats():
    ...
    logger.info(f"Query task stats: {stats['total_tasks']} tasks")
...
async def cleanup_old_tasks(max_age_hours: int = 24):
    ...
    logger.info(f"Cleanup expired tasks: {tasks_cleaned} tasks")
